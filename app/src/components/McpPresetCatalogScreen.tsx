...
import { Brain, Clock, Database, FileMagnifyingGlass, GitBranch, GlobeHemisphereWest, ChatsCircle, Notebook, HardDrives, TreeStructure } from '@phosphor-icons/react';
...
const PRESETS: McpPreset[] = [
  { id: 'notion', name: 'Notion', description: 'Páginas, bases de dados e tarefas do workspace.', category: 'OAuth', auth: 'OAuth ao usar', icon: <Notebook size={22} />, config: { type: 'http', url: 'https://mcp.notion.com/mcp' } },
  { id: 'figma', name: 'Figma', description: 'Contexto de arquivos e componentes de design.', category: 'OAuth', auth: 'OAuth ao usar', icon: <TreeStructure size={22} />, config: { type: 'http', url: 'https://mcp.figma.com/mcp' } },
  { id: 'atlassian', name: 'Atlassian', description: 'Jira, Confluence, Bitbucket e produtos Atlassian Cloud.', category: 'OAuth', auth: 'OAuth ao usar', icon: <TreeStructure size={22} />, config: { type: 'http', url: 'https://mcp.atlassian.com/v2/mcp' } },
  { id: 'slack', name: 'Slack', description: 'Mensagens, canais e arquivos do Slack.', category: 'OAuth', auth: 'OAuth do app Slack', icon: <ChatsCircle size={22} />, config: { type: 'http', url: 'https://mcp.slack.com/mcp' } },
  ...
  { id: 'filesystem', name: 'Filesystem', description: 'Arquivos e diretórios permitidos no computador.', category: 'Local', auth: 'Sem autenticação', icon: <FileMagnifyingGlass size={22} />, config: { command: 'npx', args: ['-y', '@modelcontextprotocol/server-filesystem'] } },
  ...
  { id: 'fetch', name: 'Fetch', description: 'Leitura e conversão de conteúdo da web.', category: 'Local', auth: 'Sem autenticação', icon: <GlobeHemisphereWest size={22} />, config: { command: 'npx', args: ['-y', '@modelcontextprotocol/server-fetch'] } },
  ...
  { id: 'time', name: 'Time', description: 'Conversão de fusos horários e horários locais.', category: 'Local', auth: 'Sem autenticação', icon: <Clock size={22} />, config: { command: 'npx', args: ['-y', '@modelcontextprotocol/server-time'] } },
  { id: 'sequential-thinking', name: 'Sequential Thinking', description: 'Raciocínio estruturado e iterativo para tarefas complexas.', category: 'Local', auth: 'Sem autenticação', icon: <TreeStructure size={22} />, config: { command: 'npx', args: ['-y', '@modelcontextprotocol/server-sequential-thinking'] } },
